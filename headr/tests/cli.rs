...
fn dies_bad_bytes() -> Result<()> {
    ...
    let mut cmd = cargo::cargo_bin_cmd!("headr");
...
fn run(args: &[&str], expected_file: &str) -> Result<()> {
    ...
    let mut cmd = cargo::cargo_bin_cmd!("headr");
    let output = cmd.args(args).output().expect("fail");
...
fn run_stdin(args: &[&str], input_file: &str, expected_file: &str) -> Result<()> {
    ...
    let mut cmd = cargo::cargo_bin_cmd!("headr");
    let output = cmd.write_stdin(input).args(args).output().expect("fail");
    assert!(output.status.success());
    assert_eq!(String::from_utf8_lossy(&output.stdout), expected);
